...
motor de jogos e é essencial para manter o código organizado e compreensivel. Um
exemplo típico da utilidade do despachante de eventos se dá na implementação da
resposta às entradas do usuário. Uma parte do motor de jogos deve ser dedicada a
ler os comandos de teclado, mouse, etc. Ao invés de responder explicitamente à
cada interação, o sistema de entrada simplesmente anuncia algo do tipo: *a tecla
...
[(<function handler at 0x...>, (), {}), (<function handler at 0x...>, ('pos_arg',), {'kw_arg': None})]
...
aos sinais. Sinais podem ser declarados com um segundo argumento `num_args` para
definir o número de argumentos que devem ser passados para os métodos trigger*.
...
class Signal(object):
    ...
    def _factory_listen_method(self):
        ...
        def listen_method(self, *args, **kwargs):
            ...
            wrapped = wrap_handler(handler, args, kwargs, pre_args=signal.num_args)
            ...
            if signal._num_filters == 0:
                ...
                if key is None:
                    ...
                    # Cria um elemento de controle substituindo o campo "wrapped"
                    # pelo dicionário das ids em cada filtro
                    control = ctl._control.setdefault(None, [])
                    book = ctl.book.setdefault(None, [])
                    idx = len(control) 
                    elem = ControlElem(idx, owner, ids, handler, args, kwargs)
                    control.append(elem)
                    book.append(wrap_handler(handler, args, kwargs, pre_args=signal.num_args))
                    ...
                    try:
                        ...
                        for ctl_e in ctl._control.get(None, []):
                            wrapped_i = wrap_handler(ctl_e.handler, (key,) + ctl_e.args, ctl_e.kwargs, signal.num_args)
                            ctl_e.wrapped[key] = listen_method(self, key, wrapped_i)
    ...
    def _factory_trigger_method(self):
        ...
        #trigger_method.__doc__ = trigger_method.__doc__ % {'signal': signal.name}
...
class SignalCtl(object):
    ...
    def handlers_full(self):
        return [(ctl.handler, ctl.args, ctl.kwargs) for ctl in self._control if ctl is not None]
