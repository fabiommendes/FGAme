...
class FileHandler(logging.Handler):
    ...
    def _configure(self):
        ...
        log.info('Logger: Record log in %s' % filename)
    ...
    def emit(self, message):
        ...
        if FileHandler.fd is None:
            try:
                ...
                log.exception('Error while activating FileHandler logger')
                return
...
def logger_config_update(section, key, value):
    ...
    log.setLevel(level=LOG_LEVELS.get(value))
...
#sys.stderr = LogFile('stderr', log.warning)

log_history = LoggerHistory
